...
const Navbar = () => {
  ...
  const sendVerificationOtp = async () => {
    try {
      ...
      const { data } = await axios.post(backendUrl + '/api/auth/send_verify_otp')
  ...
  return (
    <div className='w-full flex justify-between items-center p-4 sm:p-6 sm:px-24 absolute top-0'>
        <div className='flex items-center gap-2 w-28 sm:w-32 cursor-pointer' onClick={() => navigate("/")}>
          <Sparkles className='w-6 h-6 text-indigo-600' />
          <span className='font-bold text-gray-800 text-lg'>MyApp</span>
        </div>
        {userData ?
        ...
        className='cursor-pointer flex items-center gap-2 border border-gray-500
